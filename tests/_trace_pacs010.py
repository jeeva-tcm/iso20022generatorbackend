...
async def run():
    ...
    for rnd in range(8):
        ...
        print(f"rnd {rnd}: {[(e['code'],e['path'],e['message'][:60]) for e in errs if (e['code'],e['path']) not in base]}")
        try:
            comp = fix_suggester.xsd_completeness_pass(cur)
            if comp != cur: cur = comp; print("  completeness pass changed XML")
        except: pass
        try:
        ...
        print(f"  fixes: {[(f['xpath'],f['fragment_xml'][:40]) for f in fixes]}")
        if not fixes: break
    ...
    residuals = [(e["code"], e["path"], e["message"][:100]) for e in final_errs if (e["code"],e["path"]) not in base]
